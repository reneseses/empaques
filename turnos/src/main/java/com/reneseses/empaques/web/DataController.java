...
public class DataController {
    ...
    public String getUsuariosFile(HttpServletResponse response){
        ...
        for(Solicitud sol: sols){
            ...
            cell = row.createCell(1); cell.setCellValue(Integer.valueOf(sol.getUsuario().getNumero()));
            cell = row.createCell(2); cell.setCellValue(sol.getTurnos());
            rownum++;
        }
        ...
        for(Repechaje rep: reps){
            ...
            cell = row.createCell(1); cell.setCellValue(Integer.valueOf(rep.getUsuario().getNumero()));
            cell = row.createCell(2); cell.setCellValue(rep.getTurnos());
            rownum++;
        }
    ...
    public String xlsUpload(ImagenForm data, BindingResult bindingResult, @RequestParam("content") MultipartFile content, Model uiModel) {
    	...
    	try {
            ...
            for (int i = 1; i <= thisSheet.getLastRowNum(); i++) {
                ...
                sol.setUsuario(usuarioServiceImpl.findUsuarioByNumero(doc.getInt("1")));
                sol.setTurnos(doc.getString("2"));
            ...
            for (int i = 1; i <= thisSheet.getLastRowNum(); i++) {
                ...
                rep.setUsuario(usuarioServiceImpl.findUsuarioByNumero(doc.getInt("1")));
                rep.setTurnos(doc.getString("2"));
