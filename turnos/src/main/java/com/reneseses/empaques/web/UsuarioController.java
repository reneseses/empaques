...
public class UsuarioController {
    ...
    public String misTurnos(Model uiModel) {
        ...
        List<Planilla> plas = planillaServiceImpl.findPlanillaEntries(inicio, count.intValue());
        if (plas.size() == 0) {
            uiModel.addAttribute("planillas", "[]");
        ...
        uiModel.addAttribute("id", planilla.getId());
        return "member/usuarios/misturnos";
    }
    ...
    public String password(Model uiModel) {
    ...
    public String update(@Valid UsuarioUpdateForm usuarioForm, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, usuarioForm);
        ...
        if ((principal.getAuthorities().contains(new SimpleGrantedAuthority("SUBENCARGADOLOCAL")) || principal.getAuthorities().contains(new SimpleGrantedAuthority("ENCARGADOLOCAL")) || principal.getAuthorities().contains(new SimpleGrantedAuthority("ADMIN")))) {
            usuario.setDisabled(usuarioForm.getDisabled());
            usuario.setLocked(usuarioForm.getLocked());
            usuario.setNumero(usuarioForm.getNumero());
            usuario.setRegimen(usuarioForm.getRegimen());
    ...
    public String uploadImg(Model uiModel, @PathVariable("id") ObjectId id) {
        Usuario principal = (Usuario) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        List<Imagen> imagenes = imagenServiceImpl.findImagenByUsuario(principal);
    ...
    public String imagenes(Model uiModel, @PathVariable("id") ObjectId id) {
        Usuario principal = usuarioService.findUsuario(id);
        List<Imagen> imagenes= imagenServiceImpl.findImagenByUsuario(principal);
        if (imagenes.size() == 0) return "redirect:/member/usuarios/imagen/" + String.valueOf(id) + "?form";
    ...
    public String resetPassword(@Valid ForgotPasswordForm form, BindingResult result, Model uiModel) {
        if (result.hasErrors()) {
            ...
            if (!usuario.getNumero().equals(form.getNumero())) {
                uiModel.addAttribute("passForm", form);
                uiModel.addAttribute("error", "Usuario no encontrado");
                return "member/usuarios/reset";
            }
    ...
    public String findUsuariosByNumeroEquals(@RequestParam("numero") Integer numero, Model uiModel) {
    	List<Usuario> list = new ArrayList<Usuario>();
    	list.add(usuarioServiceImpl.findUsuarioByNumero(numero));
    ...
    public String findUsuariosByRutEquals(@RequestParam("rut") String rut, Model uiModel) {
    	List<Usuario> list = new ArrayList<Usuario>();
    	list.add(usuarioServiceImpl.findUsuarioByRut(rut));
        uiModel.addAttribute("usuarios", list);
        return "member/usuarios/list";
    }
}
