...
privileged aspect FaltaController_Roo_Controller {
    ...
    @Autowired
    UsuarioService FaltaController.usuarioService;
    ...
    public String FaltaController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("faltas", faltaService.findFaltaEntries(firstResult, sizeNo));
            float nrOfPages = (float) faltaService.countAllFaltas() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("faltas", faltaService.findAllFaltas());
        }
        addDateTimeFormatPatterns(uiModel);
    ...
    void FaltaController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("falta_fecha_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
    }
